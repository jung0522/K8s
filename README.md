...
저희가 서버를 2개의 언어로 구성한 이유는 각 기술의 강점과 목적에 맞춰 역할을 분리했기 때문입니다.
Flask (Python 3.11)
AI 모델, LLM 기반 퀘스트 생성, RAG 기반 혈당 분석 등 머신러닝/데이터 처리 중심
Python 생태계의 AI 라이브러리 지원이 풍부해서 빠르고 효율적으로 AI 서비스를 구현 가능

Spring Boot (Java 17)
기본적인 CRUD, 도메인 중심 설계, 보안, 로깅 등 안정적인 웹 서비스 처리
Java의 타입 안정성과 풍부한 엔터프라이즈 기능 덕분에 백엔드의 핵심 비즈니스 로직을 신뢰성 있게 관리 가능
